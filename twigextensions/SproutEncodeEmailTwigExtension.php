...
class SproutEncodeEmailTwigExtension extends Twig_Extension
{
    ...
    public function getFilters()
    {
        return array(
            ...
            'protectMailTo' => new Twig_Filter_Method($this, 'protectMailTo', array('is_safe' => array('html')))
        );
    }
    ...
    /**
     * Encode mailto links in rich text field output.
     *
     * @param  string $string Value to be encoded
     * @return mixed          Returns string encoded as HTML
     */
    public function protectMailTo($html)
    {
        ...
        foreach ($a as $anchor)
        {
            ...
            if ($anchor->hasAttribute('href'))
            {
                ...
                if (substr($href, 0, 7) == 'mailto:')
                {
                    ...
                    $email = substr($href, 7, strlen($href));

                    $string = $this->_encodeStringRot13('<a href="mailto:'.$email.'">'.$email.'</a>');

                    $html = str_replace('<a href="mailto:'.$email.'">'.$email.'</a>', $string, $html);
